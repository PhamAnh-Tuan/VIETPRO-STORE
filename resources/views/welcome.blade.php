...
class ProductController extends Controller
{
    ...
    function createPOST(CreateProductRequest $request)
    {
        ...
        $approve->prd_name = $request->name;
        $approve->prd_code = $request->code;
        $approve->prd_name = $request->name;
        $approve->prd_price = $request->price;
        $approve->prd_featured = $request->featured;
        $approve->prd_state = $request->state;
        $approve->prd_info = $request->info;
        $approve->prd_describer = $request->describer;
        $approve->prd_slug = Str::slug($request->name, '-');
        $approve->prd_image = $request->file('image')->getClientOriginalName();
        foreach ($request->cat_id as $approver) {
            $approve->cat_id = $approver;
        }
        $approve->save();
        foreach ($request->cat_id as $approver) {
            $approve->categoryy()->attach($approver);
        }
       
       // upload file ->categoryy()->attach($approver)
